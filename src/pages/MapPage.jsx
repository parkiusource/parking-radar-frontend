...
const MapPage = () => {
  ...
  const fetchSpots = useCallback(async () => {
    try {
      ...
      const uniqueData = data.filter((value, index, self) => index === self.findIndex((t) => t.id === value.id));
    ...
    <div className="app-container">
      ...
      <div className="map-container-wrapper">
        <MapContainer center={[4.711, -74.0721]} zoom={12} className="map-container">
          ...
          <TileLayer url={TILE_LAYERS.DAY} attribution='&copy; <a href="https://www.mapbox.com/about/maps/">Mapbox</a> contributors' />
          {userLocation && (
            <>
              ...
              <Marker key={spot.id} position={[spot.latitude, spot.longitude]} icon={parkingIcon}>
                <Popup>
                  <div className="popup-content">
                    ...
                    <button className="btn btn-primary mt-2" onClick={() => openNavigation(spot.latitude, spot.longitude)}>
                      Navegar aquí
                    </button>
                  </div>
                </Popup>
